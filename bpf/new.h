...
static __always_inline
int fou4_push(struct xdp_md *ctx, unsigned char *router, __be32 saddr, __be32 daddr, __u16 sport, __u16 dport)
{
    struct pointers p = {};
...
{
    //if (in6_addr_isnul(saddr))
    //	return -1;
    //if (in6_addr_isnul(saddr) || in6_addr_isnul(daddr))
    //	return -1;
